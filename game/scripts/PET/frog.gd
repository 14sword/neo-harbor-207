...
@export var catch_up_speed: float = 200.0
...
func _choose_idle_behavior():
	var behaviors = ["jump", "idle", "idle", "move_closer"]
	_idle_behavior = behaviors[randi() % behaviors.size()]
	_idle_behavior_timer = randf_range(3.0, 8.0)
	if _idle_behavior == "jump":
		if animator.sprite_frames.has_animation("jump"):
			animator.play("jump")
...
func _log_pet_action(action: String):
	var action_names = {"idle": "发呆", "jump": "跳跃"}
